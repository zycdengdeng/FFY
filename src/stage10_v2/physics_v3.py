...
设计 14 维 = v2.5 的 10 维腿 + 4 维轮：
    x[10] = wb    轮距 (m)          [0.75, 0.95]   v3_grid: 0.7 断崖（前翻），0.8 起全 Fr 存活
    x[11] = τ_max 刹车峰值力矩 (N·m) [2.0, 6.0]    v3_grid: 甜点 τ≈4；6 是翻倒边界（保留让模型见边界）
    x[12] = r_w   轮半径 (m)        [0.02, 0.045]  v3_grid: 不敏感（停车中位 1.15–1.56 m）
    x[13] = ω_th  刹车咬合转速 (rad/s) [15, 60] 对数  v3_grid: 停车主杠杆——40 vs 10 把停车从 ~100 m 压到 ~5 m
...
WB_RANGE   = (0.75, 0.95)      # m —— v3_grid 定案：0.7 是断崖（Fr≥1 翻倒/滑百米），0.8 起全 Fr 存活
TAU_RANGE  = (2.0, 6.0)        # N·m
RW_RANGE   = (0.020, 0.045)    # m
OMTH_RANGE = (15.0, 60.0)      # rad/s 对数 —— v3_grid：ω_th 是停车主杠杆，10 停不住（中位 100+ m）
